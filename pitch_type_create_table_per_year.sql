...
alter table _all_pitcher_pitch_type add primary key(year,pitcher_id,pitch_type);
...
alter table _all_pitches add primary key(year,pitcher_id,pitch_type);
...
create table pitch_type_create_table_per_year(
	select T.year as years, T.pitcher_id as id, N.first_name, N.last_name, T.pitch_type, T.v0_avg, T.v_delta_avg, T.y_delta_avg, T.spin_rate_avg
