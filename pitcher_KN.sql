/*先找出使用率超過5%的有誰*/
DROP TABLE if EXISTS pitcher_KN_use_more_than_5;
CREATE table pitcher_KN_use_more_than_5(
    SELECT
    ...
    (
        SELECT p.ab_id, 
               (case when p.pitch_type = "KN" then 1
...
/*再分析該球種*/
DROP TABLE if EXISTS pitcher_KN;
CREATE TABLE pitcher_KN(
    SELECT
    P.years,
    P.pitcher_id,
    pn.first_name,
    ...
    (
        ...
        WHERE
        p.ab_id = a.ab_id and p.pitch_type = "KN"
    )as P,
    pitcher_KN_use_more_than_5 as temp,
    player_names as pn
...
alter table pitcher_KN add primary key(years,pitcher_id);