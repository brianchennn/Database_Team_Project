select TTT1.Year,TTT1.first_name,TTT1.last_name,(OBP+SLG) as OPS
from(
select Year,first_name,last_name,sum(cnt_onbase),sum(cnt_atbat),sum(cnt_onbase)/sum(cnt_atbat) as OBP
from(
    select substring(ab_id,1,4)as Year,first_name,last_name,event,if(event="Single",count(*),if(event="Double",count(*),if(event="Triple",count(*),if(event="Home Run",count(*),if(event = "Walk",count(*),if(event="Hit By Pitch",1,if(event="Catcher Interference",count(*),if(event="Intent Walk",count(*),0)))))))) as cnt_onbase, count(*) as cnt_atbat
...
where TTT1.Year=TTT2.Year and TTT1.first_name=TTT2.first_name and TTT1.last_name=TTT2.last_name
group by TTT1.Year,TTT1.first_name,TTT1.last_name
order by TTT1.Year,OPS