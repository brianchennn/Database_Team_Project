/*先找出使用率超過5%的有誰*/
DROP TABLE if EXISTS pitcher_PO_use_more_than_5;
CREATE table pitcher_PO_use_more_than_5(
    SELECT
    ...
    (
        SELECT p.ab_id, 
               (case when p.pitch_type = "PO" then 1
               when p.pitch_type = "FO" then 1
...
/*再分析該球種*/
DROP TABLE if EXISTS pitcher_PO;
CREATE TABLE pitcher_PO(
    ...
    pn.last_name,
    P.pitch_type,
    ROUND(avg(P.start_speed),1) as PO_v0_avg,
    ROUND(avg(P.delta_speed),1) as PO_v_delta_avg,
    ROUND(avg(P.delta_y),1) as PO_y_delta_avg,
    ROUND(avg(P.spin_rate),1) as PO_spin_rate_avg
